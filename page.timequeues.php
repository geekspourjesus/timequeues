...
?>
	<h2><?php echo ($itemid ? _("Time Condition:")." ". $itemid : _("Add Time Queue Members")); ?></h2>
<?php		
	if ($itemid){ 
...
//draw goto selects
if (isset($thisItem)) {
	echo drawselects($thisItem['timequeue'],0);
} else { 
...
<select onChange="insertExten();" id="insexten" tabindex="<?php echo ++$tabindex;?>">
						<option value=""><?php if (isset($thisItem)) {echo ($thisItem['agent']);};?></option>
	<?php
...
//draw goto selects
if (isset($thisItem)) {
	echo drawselects($thisItem['agent'],1);
} else { 
